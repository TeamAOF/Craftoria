ServerEvents.recipes((e) => {
  ...
  Ingredient.of('#minecraft:logs').itemIds.forEach((id) => {
    ...
    if (Item.exists(plank)) {
      ...
      if (Item.exists(`${baseID}_hanging_sign`))
        mekanism.sawing(`2x ${plank}`, `${baseID}_hanging_sign`, 'mekanism:sawdust', 0.5);
      if (Item.exists(`${baseID}_pressure_plate`))
        mekanism.sawing(`${plank}`, `${baseID}_pressure_plate`, '2x mekanism:sawdust', 0.25);
      if (Item.exists(`${baseID}_fence_gate`))
        mekanism.sawing(`2x ${plank}`, `${baseID}_fence_gate`, '4x minecraft:stick', 1);
