  ...
  return {
    ...
    enchantingApparatus(output, input, pedestalItems, sourceAmount, keepNbt) {
      ...
      pedestalItems.forEach((pedestalItem) => {
        let [count, item] = pedestalItem.includes("x ")
          ? pedestalItem.split("x ")
          : [1, pedestalItem];
        ...
        if (pedestalItemCount > 8) {
          console.error(
            "Ars Nouveau Enchanting Apparatus pedestal items count exceeded 8. Please check the recipe."
          );
          console.error(
            `Recipe output: ${output}, Recipe input: ${input}, Recipe pedestal items: ${pedestalItems}`
          );
          return;
        }
      });
    ...
    imbuementChamber(output, input, pedestalItems, sourceAmount) {
      ...
      pedestalItems.forEach((pedestalItem) => {
        ...
        if (pedestalItemCount > 8) {
          console.error(
            "Ars Nouveau Imbuement Chamber pedestal items count exceeded 8. Please check the recipe."
          );
          console.error(
            `Recipe output: ${output}, Recipe input: ${input}, Recipe pedestal items: ${pedestalItems}`
          );
