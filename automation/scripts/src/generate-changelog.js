...
const packMetadata = (() => {
  try {
    return require("../../../packwiz/pack.toml");
  } catch {
...
Options:
  --test                    Enable test mode (disables file saving by default)
  --fromCommit <hash>       Compare from this commit hash
  --toCommit <hash>         Compare to this commit hash (default: HEAD)
  --fromVersion <version>   Override the old pack version for display
  --toVersion <version>     Override the new pack version for display
...
async function getCommitMetadataFiles(commitHash) {
  ...
  // Check which metadata file exists (new format in packwiz/ or old format in root)
  const hasNewPackToml = await checkFileExists(commitHash, "packwiz/pack.toml");
  const hasOldPackToml = !hasNewPackToml && await checkFileExists(commitHash, "pack.toml");
  const hasInstanceJson = !hasNewPackToml && !hasOldPackToml && await checkFileExists(commitHash, "minecraftinstance.json");

  const isNewFormat = hasNewPackToml;
  const isOldFormat = hasOldPackToml;
  const hasPackToml = hasNewPackToml || hasOldPackToml;
  ...
  if (hasPackToml) {
    const packTomlPath = isNewFormat
      ? `${folderPath}/packwiz/pack.toml`
      : `${folderPath}/pack.toml`;
    packMetadata = require(packTomlPath);
    modsMetadata = await extractModsFromPackwiz(folderPath, commitHash, isNewFormat);
  } else {
...
async function extractModsFromPackwiz(folderPath, commitHash, isNewFormat = true) {
  const folders = ["mods", "resourcepacks", "shaderpacks"];
  const allMods = {};
  const basePath = isNewFormat ? `${folderPath}/packwiz` : folderPath;

  for (const folder of folders) {
    try {
      const files = await Array.fromAsync(
        new Glob("*.pw.toml").scan({cwd: `${basePath}/${folder}`, absolute: true})
      );
